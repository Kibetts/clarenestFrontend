...
const MarkAttendance = () => {
    ...
    const fetchStudents = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${process.env.BACKEND_URL}/api/lessons/${selectedClass}/students`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch('${process.env.BACKEND_URL}/api/attendances', {
                method: 'POST',
                headers: {
